...
| [**uavwall**](demos/uavwall/) | Live **RTSP feeds** (UAV downlinks) picked from a rail, arranged on a canvas with grid presets and drag/resize, and pushed into a **Pexip VMR** as the participant's video. Ships a generator for synthetic test feeds, so it demonstrates itself. | C++ / CMake (macOS, Linux) · built by default |
