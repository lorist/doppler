...
| [**pexclient**](demos/pexclient/) | A complete **video client**: register to Infinity via `_pexapp` SRV, directory search, dial/receive, PIN entry, device selection, screen share, roster with host controls, chat, live captions and conference controls — styled per the "Dark Frosted" design handoff. | C++ / CMake (macOS, Windows) · built by default |
| [**uavwall**](demos/uavwall/) | Live **RTSP feeds** (UAV downlinks) picked from a rail, arranged on a canvas with grid presets and drag/resize, and pushed into a **Pexip VMR** as the participant's video. Ships a generator for synthetic test feeds, so it demonstrates itself. | C++ / CMake (macOS, Linux, Windows) · built by default |
...
sdk/        The Pulse SDK artifacts: linux/ (debs + extracted), macos/, windows/.
            Note the architecture limits — see docs/porting.md, and
            docs/third-party.md before packaging any of it.
