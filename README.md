...
| [**pexclient**](demos/pexclient/) | A complete **macOS video client**: register to Infinity via `_pexapp` SRV, directory search, dial/receive, PIN entry, device selection, screen share, roster with host controls, chat, live captions and conference controls — styled per the "Dark Frosted" design handoff. | C++ / CMake (macOS) · built by default |
